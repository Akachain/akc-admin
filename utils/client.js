...
const logger = loggerComon.getLogger('client');
hfc.setLogger(logger);

async function getClientForOrg(userorg, username) {
  logger.debug('getClientForOrg - ****** START %s %s', userorg, username);
  const config = '-connection-profile-path';
  hfc.setConfigSetting(`${userorg}-connection-profile-path`, path.join(__dirname, '../artifacts', `${userorg}.yaml`));
...
const registerUser = async (username, userOrg, isJson) => {
  try {
    ...
    if (user && user.isEnrolled()) {
      ...
      user = await fabricClient.setUserContext({
        username,
        password: secret,
      });
    ...
    throw new Error(`Failed to get registered user: ${username} with error: ${error.toString()}`);
  }
};
